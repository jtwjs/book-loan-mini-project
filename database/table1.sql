...
insert into book_manager values( 'BITCAMP','qlxmzoavm'); //유일한 관리자
...
create table BOOK_application( --도서신청 테이블
    id varchar2(15),
   ...
   CREATE SEQUENCE BACKUP_application_SEQ -- 신청이력 시퀸스
    INCREMENT BY 1
    START WITH 1
    MINVALUE 1
    MAXVALUE 1000
    NOCYCLE; 
     ...
     create table backup_application( --신청이력 테이블
  no number,
  id varchar2(15),
  book_title varchar2(20) not null,
    publisher varchar2(20) not null,
...
desc book_info;
insert into book_info values ('440','RIDE','정태웅','달콤',TO_DATE('20051212','YYYY-MM-DD'),'제1자료실',5);
